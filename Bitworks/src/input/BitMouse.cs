...
public class BitMouse
{
    ...
    {
        if (newMouse.LeftButton == ButtonState.Pressed && oldMouse.LeftButton != ButtonState.Pressed && newMouse.Position.X >= 0 && newMouse.Position.X <= BitworksGame.Graphics.PreferredBackBufferWidth && newMouse.Position.Y >= 0 && newMouse.Position.Y <= BitworksGame.Graphics.PreferredBackBufferHeight)
        {
            return true;
        }
    ...
    {
        Vector2 screenPos = newMousePos;

        Vector2 screenCenter = new Vector2(
            BitworksGame.Graphics.PreferredBackBufferWidth / 2f,
            BitworksGame.Graphics.PreferredBackBufferHeight / 2f
        );

        return (screenPos - screenCenter) / BitworksGame.Camera.Zoom + BitworksGame.Camera.Position;
    }
    ...
    {
        if (newMouse.RightButton == ButtonState.Pressed && oldMouse.RightButton != ButtonState.Pressed && newMouse.Position.X >= 0 && newMouse.Position.X <= BitworksGame.Graphics.PreferredBackBufferWidth && newMouse.Position.Y >= 0 && newMouse.Position.Y <= BitworksGame.Graphics.PreferredBackBufferHeight)
        {
            return true;
        }
